...
const test = async () => {
  ...
  const runtimeArgs = RuntimeArgs.fromMap({
    contract_name: CLValueBuilder.string(CONTRACT_FACTORY_NAME!),
    // admin: CLPublicKey.fromHex(ADMIN!),
  });

  try {
    let installDeployHash = await contract.install(
      getBinary(WASM_FACTORY_PATH!),
      runtimeArgs,
      INSTALL_PROFILE_PAYMENT_AMOUNT!,
      KEYS.publicKey,
      CHAIN_NAME!,
    ...
    const hash: any = await installDeployHash?.send(NODE_ADDRESS!);
    ...
    const contractHash = await getAccountNamedKeyValue(
      accountInfo,
      `${CONTRACT_FACTORY_NAME!}_contract_hash`
    );

    const contractPackageHash = await getAccountNamedKeyValue(
      accountInfo,
      `${CONTRACT_FACTORY_NAME}_contract_package_hash`
    );
